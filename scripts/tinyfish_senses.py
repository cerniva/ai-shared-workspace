...
def task_record(ledger: dict, task_id: str) -> dict | None:
    value = ledger.get(task_id)
    return value if isinstance(value, dict) else None


def record_run_start(ledger: dict, task: dict[str, object], run_id: str) -> dict:
    task_id = str(task.get("id", ""))
    previous = task_record(ledger, task_id) or {}
    record = {
        "task_id": task_id,
        "requested_by": str(task.get("requested_by", "chatgpt")),
        "mode": str(task.get("mode", "fetch")),
        "status": "running",
        "run_id": run_id,
        "updated_at": now(),
        "last_error": "",
        "routed_event_keys": list(previous.get("routed_event_keys", [])),
    }
    ledger[task_id] = record
    return record


def record_terminal(ledger: dict, task_id: str, status: str, last_error: str = "") -> dict:
    record = dict(task_record(ledger, task_id) or {"task_id": task_id})
    record.update({"status": status, "updated_at": now(), "last_error": last_error})
    record.setdefault("requested_by", "chatgpt")
    record.setdefault("mode", "fetch")
    record.setdefault("run_id", "")
    record.setdefault("routed_event_keys", [])
    ledger[task_id] = record
    return record
...
def parse_task(block: str) -> dict[str, object]:
    ...
    if mode == "fetch" and not urls:
        urls = [DEFAULT_FETCH_URL]
    return {
...
def append_result(task: dict[str, object], status: str, data: object) -> None:
    serialized = json.dumps(data, ensure_ascii=False, default=str)[:MAX_RESULT_CHARS]
    block = (f"\n---\nid: TF-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}\n" f"task_id: {task.get('id', '')}\nfrom: tinyfish\nrequested_by: {task.get('requested_by', 'chatgpt')}\n" f"mode: {task.get('mode', 'fetch')}\nstatus: {status}\ncreated_at: {now()}\n---\n\n" f"```json\n{serialized}\n```\n")
...
def append_action_once(service: str, reason_code: str, reason: str) -> bool:
    ...
    for block in re.split(r"(?m)^---\s*$", text):
        if re.search(r"(?im)^status:\s*open\s*$", block) and re.search(rf"(?im)^service:\s*{re.escape(service)}\s*$", block) and re.search(rf"(?im)^reason_code:\s*{re.escape(reason_code)}\s*$", block):
            return False
    record = (f"\n---\nid: ACTION-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-tinyfish\nsource: tinyfish-worker\n" f"status: open\nservice: {service}\nreason_code: {reason_code}\ncreated_at: {now()}\n---\n\n" f"## BAĞLANTI GEREKİYOR\n- Servis: {service}\n- Secret adı: TINYFISH_API_KEY\n- Neden: {reason}\n")
    ACTION.write_text(text + record, encoding="utf-8")
    return True
...
def execute_task(task: dict[str, object], key: str, ledger: dict | None = None) -> tuple[str, object, str]:
    task_id = str(task.get("id", ""))
    existing = task_record(ledger, task_id) if ledger is not None else None
    if existing and existing.get("status") in TERMINAL:
        return str(existing["status"]), {"run_id": existing.get("run_id", ""), "reused": True}, str(existing.get("last_error", ""))
    if task.get("mode") == "browser" and existing and existing.get("status") in {"running", "retryable"} and existing.get("run_id"):
        return str(existing["status"]), {"run_id": existing["run_id"], "reused": True}, str(existing.get("last_error", ""))
    try:
        if task.get("mode") == "fetch":
            data = fetch(task["urls"], key)
            if ledger is not None:
                record_run_start(ledger, task, "")
                record_terminal(ledger, task_id, "done", "")
            return "done", data, ""
        data = run_browser(task, key)
        run_id = str(data.get("run_id") or data.get("id") or "") if isinstance(data, dict) else ""
        if not run_id:
            reason = "TinyFish browser response missing run_id"
            if ledger is not None:
                record_run_start(ledger, task, "")
                record_terminal(ledger, task_id, "blocked", reason)
            return "blocked", {"reason_code": "missing-run-id"}, reason
        if ledger is not None:
            record_run_start(ledger, task, run_id)
        return "running", {"run_id": run_id, "remote": data}, ""
    except urllib.error.HTTPError as exc:
        reason_code, reason = classify_http_status(exc.code)
        status = "retryable" if reason_code == "transient" else "blocked"
        if ledger is not None:
            record_run_start(ledger, task, str((existing or {}).get("run_id", "")))
            record_terminal(ledger, task_id, status, reason)
...
def main() -> int:
    block = latest_task(INBOX.read_text(encoding="utf-8") if INBOX.exists() else "")
    task = parse_task(block)
    if task.get("status") not in {"queued", "ready", "open", "run"}: return 0
    valid, reason = validate_task(task)
    if not valid:
        append_result(task, "failed", {"reason": reason}); mark_inbox("failed", reason); return 0
    safety_reason = browser_block_reason(task) if task.get("mode") == "browser" else ""
    if safety_reason:
        append_result(task, "failed", {"reason": safety_reason}); mark_inbox("failed", safety_reason); return 0
    key = os.environ.get("TINYFISH_API_KEY", "").strip()
    if not key:
        append_action_once("TinyFish", "missing-secret", "TINYFISH_API_KEY missing; worker skipped."); append_result(task, "blocked", {"reason_code": "missing-secret"}); mark_inbox("blocked", "missing TINYFISH_API_KEY"); return 0
    ledger = load_ledger()
    status, data, reason = execute_task(task, key, ledger=ledger)
    save_ledger(ledger)
    append_result(task, status, data)
